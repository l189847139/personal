/*
 ...
 8、针对万得业务主键字段值直接变更导致产生脏数据的表，清洗任务改用object_id做清洗，改唯一索引为普通索引，融合需加新逻辑或者模块处理【待处理-刘江】
 9、日志删除任务是否考虑调整调度为顺序调度或者调度模式可选，当前为周期调度，考虑到后续需要任务越来越多【待确认】
 10、部分万得表在S_INFO_WINDCODE临时代码变更为正式代码时，变更时间与WINDCUSTOMCODE的变更时间过于接近，
 导致清洗时关联不到，考虑将清洗和融合时的推移天数单位改为小时/分钟，且清洗任务source表的推移时间无法配置【待处理-林波涛】
 11、离线清洗增量回撤时间单位调整为分钟【待处理-刘波】
 12、主数据编码变更，中间表和融合表传参反了【已完成-刘波】
 13、清洗融合任务失败时的预警机制日志/数据库 【待处理-林波涛】
 14、主数据证券模型针对全称、简称、上市日期等非主数据模型表获取的字段西悉尼进行对应证券类型的指定表的映射字段监控同步（或通过配置flink清洗任务）【待处理-林波涛 】
