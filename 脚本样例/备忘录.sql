/*
 1、2025.12.1李润物调整过日志打印，需更新log4j2-prod.xml(测试是log4j2-dev.xml)
 2、2025.12.2客户需要提供平台操作文档，和系统培训客户方成员便于后期运维（任务开发，数据排查）
 3、2025.12.4记录问题：当主数据编码发生变更时，会将编码同步变更到融合表，但是不会变更交易代码。影响：行情表中的交易代码没变更，行情表模型设计了TRD_CODE，业务经常直接使用，导致数据少了(实际未少)
 例如融资融券明细，业务直接使用trd_code，920725只有今年8月之后的行情，源头上有8月之前的，融合数据的SECU_ID发生了变更，但是未同步变更trd_code导致
 4、增加针对调度的统计和监控-运营大屏【待处理】
 5、邮件预警的配置【待处理】
 6、万得表直接变更业务主键字段，导致多源融合的资讯清洗融合数据产生冗余数据【万得问题，可忽略】
 7、清洗融合任务失败时的邮件预警机制【待处理】
 8、融合血缘关系没有被记录【待处理】
 9、针对万得业务主键字段值直接变更导致产生脏数据的表，清洗任务改用object_id做清洗，改唯一索引为普通索引，融合需加新逻辑或者板块处理【待处理】
 10、日志删除任务是否考虑调整调度为顺序调度或者调度模式可选，当前为周期调度，考虑到后续需要任务越来越多【待确认】
 11、部分万得表在S_INFO_WINDCODE临时代码变更为正式代码时，变更时间与WINDCUSTOMCODE的变更时间过于接近，导致清洗时关联不到，考虑将清洗时的推移天数单位改为小时/分钟
...
WHERE
    IS_VALID = 1
    AND '022124' = TRD_CODE
    AND '20250320' = TO_CHAR(LST_DATE, 'YYYYMMDD')
    AND TYP_CODE LIKE 'EQD%'
    AND IS_VALID = 1
UPDATE
    MAPPING_SECURITIES
SET
